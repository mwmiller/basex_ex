   ...
   quote bind_quoted: binding()  do
        defmodule name do
          ...
          defp chars_to(<<c::binary-size(1)>>, final_size, acc) do
            ...
            cond do
                  final_acc > max -> decode_error("value too large")
                  true            -> final_acc |> :binary.encode_unsigned(:big)
                 end
              |> pad_chars(final_size)
          end
          defp chars_to(<<c::binary-size(1),rest::binary>>, final_size, acc), do: chars_to(rest, final_size, (acc+char_val(c)) * unquote(a))

          defp pad_chars(b, final_size) when bit_size(b) == final_size, do: b
          defp pad_chars(b, final_size) when bit_size(b) <  final_size, do: pad_chars(<<0>><>b, final_size)
